...
def alex_net_model(_X, _weights, _biases, _dropout):
...
# Construct model
pred = alex_net_model(img_pl, weights, biases, keep_prob)
...
with tf.Session() as sess:
    ...
    for epoch in xrange(max_epoch):
        for step in xrange((len(imgs)/BATCH_SIZE) +1):
            ...
            batch_imgs, batch_labels = nextBatch(imgs, labels, step, BATCH_SIZE)
